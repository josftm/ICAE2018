...
#h2o.init(nthreads=6, max_mem_size = "16G", startH2O = TRUE)
...
dir = "/home/josftm/Documentos/lab/SparkR/Experimentacion_ICAE/escalabilidad/2hilos"

#for(factor.rep in c("matriz_x1","matriz_x2","matriz_x4","matriz_x8","matriz_x16","matriz_x32","matriz_x64")){
for(factor.rep in c("matriz_x2")){
  #Crear conexion con Spark
  sc <- spark_connect(master = "local[6]", version = "2.0.2",
                      config = list("sparklyr.shell.executor-memory"="16g", 
                                    "sparklyr.shell.driver-memory"="16g",
                                    "sparklyr.cores.local"="spark.driver.cores=6",
                                    "sparklyr.cores.local"="6",
                                    "sparklyr.shell.num-executors"="6",
                                    "sparklyr.shell.executor-cores"="6",
                                    "spark.executor.cores"="6",
                                    "sparklyr.shell.total-executor-cores"="6",
                                    "spark.ext.h2o.nthreads"="6",
                                    "spark.cores.max"="6")
  )
  ...
  data_df <- spark_read_csv(sc, "dataset", sprintf("/home/josftm/Documentos/lab/SparkR/Datos/electricidad_spain/LOCAL_H24_W168_12HILOS/%s/redistribuido.csv",factor.rep),
